...
impl<'a> BinaryReader<'a> {
    ...
    pub fn read_string(&mut self) -> anyhow::Result<String> {
        ...
        if length > 1024 * 1024 * 128 { // 128MB safety limit for a single string
            anyhow::bail!("read_string: requested {} bytes, which exceeds 128MB limit", length);
        }
    ...
    pub fn read_cstring(&mut self) -> anyhow::Result<String> {
        ...
        loop {
            ...
            buf.push(b);
            count += 1;
            if count > 4096 { break; } // Safety limit
        }
        Ok(String::from_utf8_lossy(&buf).into_owned())
    }
    ...
    pub fn read_bytes(&mut self, n: usize) -> anyhow::Result<Vec<u8>> {
        if n > 1024 * 1024 * 1024 { // 1GB safety limit
            anyhow::bail!("read_bytes: requested {} bytes, which exceeds 1GB limit", n);
        }
