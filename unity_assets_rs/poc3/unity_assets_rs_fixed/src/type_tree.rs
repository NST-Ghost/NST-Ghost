...
impl TypeTree {
    pub fn read(
        ...
        for i in 0..count {
            ...
            if version >= 16 { reader.read_u8()?; }  // is_stripped

            let script_index: i16 = if version >= 17 { reader.read_i16()? } else { -1 };

            if version >= 13 {
                // MonoBehaviour (class 114) has an extra script GUID
            ...
            if has_type_tree {
                // FIX #1: insert with class_id, not loop index i.
                // ObjectInfo::type_id maps to class_id, not to the ordinal.
                let tree = Self::read_nodes(reader, version)?;
                types.insert(class_id, tree);
            } else {
                // No node data to read; record an empty placeholder so callers
                // can still look up the class.
                types.insert(class_id, TypeTree { nodes: vec![], string_buffer: vec![] });
            }

            let _ = (i, script_index); // suppress warnings
        }

        // FIX #2: only read the v21+ ref-types block when has_type_tree is true.
        // When has_type_tree=false the block is absent and reading it would
        // consume bytes that belong to the object table.
        if version >= 21 && has_type_tree {
            ...
            for _ in 0..ref_count {
                ...
                if version >= 13 {
                    // FIX #3: use class_id == 114, not scr_idx >= 0
