...
impl SerializedFile {
    pub fn read(data: &[u8]) -> Result<Self> {
        // ── Step 1: fixed header (always big-endian) ───────────────────────────
        let mut hr = BinaryReader::new(data, Endian::Big);
        let ms_legacy   = hr.read_u32()?;   // offset  0
        let fs_legacy   = hr.read_u32()?;   // offset  4
        let version     = hr.read_u32()?;   // offset  8
        let do_legacy   = hr.read_u32()?;   // offset 12
        let endian_byte = hr.read_u8()?;    // offset 16
        hr.skip(3)?;                        // offset 17 reserved

        // FIX: use correct fields depending on version
        let (metadata_size, file_size, data_offset) = if version >= 22 {
            let ms  = hr.read_u32()?;  // offset 20
            let fs  = hr.read_u64()?;  // offset 24
            let dof = hr.read_u64()?;  // offset 32
            let _   = hr.read_u64()?;  // offset 40 unknown
            (ms, fs, dof)
        } else {
        ...
        // v22+: header is 48 bytes.  Older: 16 bytes.
        let meta_start: u64 = if version >= 22 { 48 } else { 16 };
        let mut meta = BinaryReader::new(data, Endian::Little);
        ...
        // FIX: pass has_type_tree so ref-types block is skipped when absent
        let types = TypeTree::read(&mut meta, version, has_type_tree)?;
        println!("Unity {} | platform {} | has_type_tree={} | {} types",
            unity_version, target_platform, has_type_tree, types.len());

        // FIX: no pre-count alignment; cursor is already correct after TypeTree::read.
        //      (The old hardcoded pattern-scan is removed entirely.)
        let objects   = ObjectInfo::read_table(&mut meta, version)?;
        println!("Read {} objects", objects.len());
