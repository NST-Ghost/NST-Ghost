...
impl TypeTree {
    ...
    pub fn read(
        ...
        for i in 0..count {
            ...
            // v16+: stripped flag (in types, not objects)
            if version >= 16 {
                let _stripped = reader.read_u8()?;
            }

            // v17+: MonoBehaviour script index (i16)
            let script_index: i16 = if version >= 17 {
                reader.read_i16()?
            } else {
                -1
            };

            // v13+: optional 16-byte script GUID + 16-byte type hash
            if version >= 13 {
                // Script GUID present when:
                //   v16+  → class_id == 114 (MonoBehaviour)
                //   <v16  → class_id < 0
                let has_guid = if version >= 16 {
                    class_id == 114
                } else {
                    class_id < 0
                };
                if has_guid { reader.skip(16)?; }
                reader.skip(16)?; // type_hash always 16 bytes
            }

            if has_type_tree {
                let nodes = Self::read_nodes(reader, version)?;
                // Key = type index (0-based), matching ObjectInfo.type_id
                types.insert(i as i32, nodes);
            }
        }

        // v21+ ref-types block exists ONLY when has_type_tree is true
        if version >= 21 && has_type_tree {
            ...
            for _ in 0..ref_count {
                ...
                if version >= 13 {
                    if scr_idx >= 0 { reader.skip(16)?; } // script GUID
