...
impl SerializedFile {
    ...
    pub fn read(data: &[u8]) -> Result<Self> {
        ...
        let (metadata_size, file_size, data_offset) = if version >= 22 {
            // Extended 64-bit header continues from current position (20) in the
            // initial big-endian reader — do NOT create a new LE reader at offset 16.
            let ms  = header_reader.read_u32()?;
            let fs  = header_reader.read_u64()?;
            let dof = header_reader.read_u64()?;
            let _unknown = header_reader.read_i64()?;
            (ms, fs, dof)
        } else {
        ...
        // endian_byte == 0 → little-endian (the "swap" flag is inverted: 0 = no-swap = LE on x86)
    ...
    pub fn endian(&self) -> Endian {
        if self.header.endian == 0 { Endian::Little } else { Endian::Big }
    }
