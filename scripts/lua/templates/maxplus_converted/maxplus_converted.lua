...
-- Version: 1.0.0
-- Author: Community
-- Description: MaxPlus AI LLM Translation Plugin via OpenAI-compatible API
...
function on_text_extract(text)
    ...
    if not api_key or api_key == "" then
        api_key = os.getenv("MAXPLUS_AI_API_KEY") or os.getenv("OPENAI_API_KEY")
    end

    if not api_key or api_key == "" then
        return nil, "Error: MaxPlus AI API Key is not configured."
    end
