...
-- Version: 1.1.0
-- Author: NST Team
-- Description: Groq API Translation Plugin featuring ultra-fast inference with Llama, Gemma, and Qwen models.
...
local function request_with_retry(url, method, headers, body)
    ...
    for attempt = 1, max_retries + 1 do
        ...
        elseif status == 429 then
            ...
            return response_body, status, "Groq HTTP Error Status: " .. tostring(status) .. " - " .. (response_body or "nil")
        end
    end
    return nil, 0, "Groq max retries exceeded"
end
...
function on_text_extract(text)
    ...
    if not api_key or api_key == "" then
        return nil, "Error: Groq API Key is not configured."
    end
    ...
    return nil, "Failed to parse Groq API response"
end
    ...
    if response and response.choices and response.choices[1] and response.choices[1].message then
        ...
        if type(translated_array) == "table" then
            if #translated_array == #text_array then
                ...
                nst_log("[Groq] Batch length mismatch: expected " .. #text_array .. ", got " .. #translated_array)
                return nil, "Groq batch length mismatch: expected " .. #text_array .. ", got " .. #translated_array
            end
        else
            nst_log("[Groq] JSON decode failed for batch content: " .. tostring(content))
        end
    end
    
    return nil, "Failed to parse Groq batch response"
end
