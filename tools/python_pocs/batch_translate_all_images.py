...
print("Initializing EasyOCR (Japanese + English)...")
...
def process_image(file_path):
    ...
    for bbox, text, prob in ocr_results:
        ...
        if prob >= 0.25 and len(text_str) >= 2:
            valid_detections.append((bbox, text_str, prob))

    if not valid_detections:
        return 0

    print(f"  ✨ Found {len(valid_detections)} text regions in {fname}")
...
def process_game(game_dir, game_name, emulator_target_path):
    print(f"\n==================================================")
    print(f"🚀 Batch Translating ALL Story Graphics in {game_name}")
    print(f"==================================================")
...
def main():
    ...
    print(f"\n✨ ALL GAME STORY GRAPHICS TRANSLATED IN {time.time() - t0:.2f} SECONDS!")
